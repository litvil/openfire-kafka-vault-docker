...
initialize_data_dir() {
  ...
  sudo -HEu ${OPENFIRE_USER} find ${OPENFIRE_DATA_DIR} -name \*.jar -exec ln -sf {} /usr/share/openfire/lib/ \;
...
if [[ -z ${1} ]]; then
  ...
  echo "Starting daemon Openfir. Second option ..."
  exec "$@"
fi
